...
describe("axis normalization", () => {
  ...
  it("removes deadband and rescales remaining travel", () => {
    const withDeadband = {
      ...calibration,
      minimum: -1,
      maximum: 1,
    ...
    expect(normalizeCenteredAxis(0.08, withDeadband)).toBe(0);
    expect(normalizeCenteredAxis(0.55, withDeadband)).toBeCloseTo(0.5);
    expect(normalizeCenteredAxis(-0.55, withDeadband)).toBeCloseTo(-0.5);
  });
